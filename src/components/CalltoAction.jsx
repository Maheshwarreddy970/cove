    ...
    <section className=' relative overflow-hidden bg-[#232526] dark:bg-[#FFFFF0]'>
      <Smoothup className='  flex flex-col gap-6 text-white bg-[#232526] dark:bg-[#FFFFF0] dark:text-black my-36 lg:my-24 '>
        ...
        <div className=' flex flex-col px-6 sm:px-0  sm:flex-row z-30 justify-center items-center gap-4 mt-[3.2rem]'>
          <a href={data.herosection.buttonslinks.appstore} className='z-30 text-base text-center w-full sm:w-auto font-semibold px-10 py-3  p-3 rounded-full border border-black bg-gradient-to-r from-[#6ACE57] to-[#07C8E6] hover:from-[#07C8E6] hover:to-[#6ACE57] active:from-[#07C8E6] active:to-[#6ACE57] text-black flex gap-3 items-center justify-center transition-all ease-out duration-300'><icons.appleicon></icons.appleicon> Get it on App Store</a>
          <a href={data.herosection.buttonslinks.playstore} className=' z-30 text-base text-center w-full sm:w-auto font-semibold px-10 py-3  p-3 rounded-full border  border-black bg-white hover:bg-white/90 text-black flex gap-3 items-center justify-center transition-all ease-out duration-300'><icons.playstoreicon></icons.playstoreicon> Get it on Play Store  </a>
        </div>
      </Smoothup>
      ...
      <motion.img
        ...
        src={'/bananatree.png'} alt='bananatree' className=' hidden lg:absolute lg:block w-[40rem] h-[40rem]  -bottom-1 -right-60 ' ></motion.img>
      <motion.img
         initial={{ x: -80 }}
         whileInView={{ x: 0 }}
         viewport={{ once: true }}
         transition={{ duration: 1.5, ease: "easeInOut", type: 'spring', stiffness: 50 }}
      src={'/tree.png'} alt='tree' className='w-[43rem] h-[43rem] object-cover -bottom-1 -left-[4rem] hidden lg:absolute lg:block '  ></motion.img>
      <motion.img
        initial={{ y: 80 }}
        whileInView={{ y: 0 }}
        viewport={{ once: true }}
        transition={{ duration: 1.5, ease: "easeInOut", type: 'spring', stiffness: 50 }}
        src={'/orginal.png'} alt='bananatree' className=' absolute md:-top-8 -top-16 -left-24 scale-150 sm:scale-100 md:-left-16 xl:left-36 w-[15.1875rem] h-[15.1875rem]  -bottom-1 -right-60 ' ></motion.img>
      <motion.img
        initial={{ y: 80 }}
        whileInView={{ y: 0 }}
        viewport={{ once: true }}
        transition={{ duration: 1.5, ease: "easeInOut", type: 'spring', stiffness: 50 }}
        src={'/orginal.png'} alt='bananatree' className=' absolute top-0 xl:-top-10 blur-sm -right-6 md:right-0 w-[8.25rem] h-[8.25rem] scale-150 sm:scale-100' ></motion.img>
